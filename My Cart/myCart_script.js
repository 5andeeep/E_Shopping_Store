  ...
  Arr.forEach((ele, index) => {
    itemContainer.innerHTML += `
          ...
          <div class="info">
            ...
            <div style='margin-top:10px;' class="row">Rating: ${Math.floor(
              ele.rating.rate
            )}</div>
          </div>
          <button id="addBtn" onClick='removeFromCart(${
            ele.id
...
//   console.log(itemToRemove);
  ...
  // updated cart after removing item
...
function totalPriceFunc() {
  return cartItem.reduce((acc, item) => {
    return Math.round((acc + item.price)*100)/100;
  }, 0);
}
...
document.getElementById("rzp-button1").onclick = function (e) {
  var options = {
    key: "rzp_test_YMQ2Yad8gzpVZh", // Enter the Key ID generated from the Dashboard
